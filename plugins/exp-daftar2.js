let handler = async function (m, { text, usedPrefix, command }) {
    ...
    if (user.registered === true) return m.reply('Akun Kamu Sudah Terverifikasi dan terdaftar di *DATABASE* Smart Bot')
...
handler.all = async m => {
    ...
    if (m.text == conn.verify[m.sender].code && m.quoted.id == conn.verify[m.sender].key) {
        conn.sendFile(m.chat, flas, 'nih.jpg', '*Terima Kasih karena anda sudah memverifikasi akun anda di DATABASE SMART BOT*', m)
        m.reply('Silahkan ketik *.help* Jika anda tidak mengerti cara penggunaan BOT, atau *.allmenu* untuk menikmati berbagai fitur yang tersedia di *Smart Bot*')
